...
function transformToLis(obj){
  let object=Object.entries(obj).map((item)=>
    `<li>${item.join(': ')}</li>`
...
function count(target, input) {
  let counter=0;  
     input.map(ele=>{  
      ele.map(item=>{if(item === target)
          counter = counter+ 1  }) 
        });
        return counter;
}
...
let findMaleAndFemale = (data) => {
  ...
  data.map(character =>{
   if(character.gender==='female' || character.gender==='male'){
     resArry.push(character.name);
   }
  })
  
  result = resArry.join(' and ');
...
let findShortest = (data) => {
  ...
  let newArr = data[0];
  data.map(item => {
    if(parseInt(item.height) <= parseInt(newArr.height)){
      newArr = item
    }
  });
  return newArr.name;
