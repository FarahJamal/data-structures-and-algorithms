...
const toLastNames = people => {
  ...
  arr2.push(element.firstName+" "+element.lastName);
  });
  return arr2;
  };
...
const addPurchases = (arr) => {
  let purSum=arr.reduce(function(a,b){
   a=a+b.purchasePrice;
   return a;
...
const returnNames = (arr) => {
  let data=arr.reduce(function(a,b,index){
    a[index]=(b.name);
  return a;
  },[])
  return data;
...
const countNumberOfChildren = (arr) => {
  let sum=0;
  let count = arr.reduce(function(a,b,i){
    if(b.children !== undefined){
             sum= b.children.length+sum;
    }
    return sum;
  },0)
  return count;
