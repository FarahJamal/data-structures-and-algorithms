...
const addOne = (arr) => {
  let arr2 = arr.map((element) => element + 1);
  return arr2;
};
...
const addQuestion = (arr) => {
  let arr2 = arr.map((element) => `${element}?`);
  return arr2;
};
...
const extractAbilities = (arr) => {
  let arrayScores = snorlaxAbilities.abilities.map(el => el.ability.name);
 return arrayScores;
 };
...
const extractStats = (arr) => {
  let arrayScores = snorlaxStats.stats.map(
  o => { return {name: o.stat.name, total: o.effort+o.baseStat} }
  );
